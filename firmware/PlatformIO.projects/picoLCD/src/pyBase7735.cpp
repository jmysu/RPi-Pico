...
#include <SPI.h>
  ...
  #define TFT_CS         PICO_CS
  #define TFT_RST        PICO_RST // Or set to -1 and connect to Arduino RESET pin
  #define TFT_DC         PICO_DC
#endif
...
arduino::MbedSPI mySPI(PyBASE_MISO, PyBASE_MOSI, PyBASE_SCK);
SPIClass* spi = &mySPI;
Adafruit_ST7735 tft = Adafruit_ST7735(spi, PyBASE_CS, PyBASE_DC, PyBASE_RST);
...
//Adafruit_ST7735 tft = Adafruit_ST7735(PyBASE_CS, PyBASE_DC, PyBASE_MOSI, PyBASE_SCK, PyBASE_RST);
//
// connect a push button between ground and...
//const uint8_t   Button_pin              = 15; //PicoKitB
const uint8_t   Button_pin              = 14; //PyBASE
...
void setup() {
    ...
    pinMode(PICO_BL, OUTPUT);
    digitalWrite(PICO_BL, HIGH);
